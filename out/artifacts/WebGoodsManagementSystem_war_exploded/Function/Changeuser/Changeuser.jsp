...
<%@ page import="functioninterface.Changeuser" %>
<%@ page import="functioninterfaceimp.Changeuserimp" %>
...
<div class="container-fluid  text-center" style="margin:auto;padding:150px;width: 500px;height: 500px">
    <form action="http://localhost:8080/WebGoodsManagementSystem_war_exploded/Function/Changeuser/Changeuser.jsp" method="post">
        <label for="name" class="form-label mt-5">名称:</label>
        <input type="text" name="name" id="name" placeholder="请输入新的用户名称" class="text-center "style="width: 250px;height: 50px"required><br>
        <label for="account"class="form-label mt-5">账号:</label>
        <input type="text" name="account" id="account" placeholder="请输入手机号码" class="text-center "style="width: 250px;height: 50px"required><br>
        <label for="realaccount"class="form-label mt-5">账号:</label>
        <input type="text" name="realaccount" id="realaccount" placeholder="请输入新的手机号码" class="text-center "style="width: 250px;height: 50px"required><br>
        <label for="password" class="mt-5 form-label">密码:</label>
        <input type="password" name="password" id="password" placeholder="请输入新的用户密码" class="text-center "style="width: 250px;height: 50px"required>
        <div class="bg-primary mt-3"style="width: 250px;height:auto">更改权限</div><br>
        普通管理员<input class="mt-3" type="radio"name="power" value="Client">   高级管理员<input type="radio" name="power" value="Manager">
        <button type="submit" class="btn-primary mt-5 ms-3"style="width: 150px ">更改</button>
    </form>
</div>
</body>
<body>
<%
    ...
    if(new Queryuserimp().queryone(request.getParameter("account"))==null)
    {
        out.print("<script>\n" +
                "    document.getElementById(\"warn\").innerHTML=\"<div class=\\\" bg-danger text-white text-center\\\" style=\\\"width:250px\\\">此用户不存在</div>\"\n" +
                "</script>");
    }
 else {
        Changeuser c=new Changeuserimp();
        c.change(request.getParameter("name"),request.getParameter("realaccount"),request.getParameter("account"),request.getParameter("password"),request.getParameter("power"));
        response.sendRedirect("http://localhost:8080/WebGoodsManagementSystem_war_exploded/Function/Userinformation/Userinformation.jsp");
    }
%>
</body>
</html>
