...
<div class="container-fluid  text-center" style="margin:auto;padding:150px;width: 500px;height: 500px">
    <form method="post" action="http://localhost:8080/WebGoodsManagementSystem_war_exploded/Function/Query/Query.jsp">
        <label for="name" class="form-label mt-5">名称:</label>
        <input type="text" name="name" id="name" placeholder="请输入商品名称" class="text-center "style="width: 250px;height: 50px"><br>
...
<%
  ...
  Goods goods=q.query(name);
    if(null==goods)
    {
        out.print("<h1 class=\"bg-danger text-center\">你查询的商品不存在</h1>");
    }
    else {
        out.print("<h1 class=\"bg-success text-center\">你查询的商品为"+goods.getName()+"价格为"+goods.getPrice()+"所剩件数为"+goods.getNumber()+"</h1>");
    }
%>
</body>
</html>
