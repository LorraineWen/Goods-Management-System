...
<%@ page import="JDBCutils.JDBCutilssituation" %><%--
...
<%
    String sql="select*from situation.situation";
    Statement stat= JDBCutilssituation.getStatement(sql);
    ...
    while(re.next()!=false)
    {
        if(flag==1)
        {
            out.print("<div class=\"text-center  h1 text-dark bg-primary\">系统使用情况</div>");
            ...
            out.println("<th ><tr class=\"h3\"><td>用户名称"+"</td><td>用户账号"+"</td><td>使用的功能</td><td>使用的时间"+"</td></tr></th>");
            flag++;
        }

        out.println("<tr class=\"h3\"><td>"+re.getString("name")+"</td><td>"+re.getString("account")+"</td><td>"+re.getString("fun")+"</td><td>"+re.getString("time")+"</td></tr>");
